...
test("the adapter consumes the centralized engine contract", async () => {
  ...
  assert.match(
    adapter,
    /assertPlaybackEngine[\s\S]+from "\.\/engine_contract\.js"/,
  );
  assert.match(
    adapter,
...
test("media-element engines validate through the same contract", async () => {
  ...
  assert.match(
    mediaElement,
    /assertPlaybackEngine\(new MediaElementEngine\(options\)/,
  );
  assert.match(native, /createMediaElementEngine/);
  assert.match(
    native,
    /assertPlaybackEngine\(new NativePlaybackEngine\(options\)/,
  );
});
