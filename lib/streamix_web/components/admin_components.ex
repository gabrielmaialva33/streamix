defmodule StreamixWeb.AdminComponents do
  ...
  def admin_tabs(assigns) do
    ...
    <nav class="flex gap-1 border-b border-border mb-6">
      <.admin_tab path={~p"/admin"} label="Dashboard" icon="hero-chart-bar" current_path={@current_path} />
      <.admin_tab path={~p"/admin/plans"} label="Planos" icon="hero-credit-card" current_path={@current_path} />
      <.admin_tab path={~p"/admin/users"} label="Usuários" icon="hero-users" current_path={@current_path} />
    </nav>
    """
  end
  ...
  defp admin_tab(assigns) do
    ...
    <.link
      ...
      class={[
        ...
        !@active && "border-transparent text-text-secondary hover:text-text-primary hover:border-border"
      ]}
    >
  ...
  def status_badge(assigns) do
    assigns =
      assign(assigns, :colors, case assigns.status do
