defmodule StreamixWeb.Helpers.ResizeUrl do
  ...
  unconditionally.
  """
  @spec flatten(String.t(), String.t() | nil, [width()]) :: map()
  ...
  def flatten(prefix, url, widths)
    ...
    Enum.reduce(widths, %{}, fn w, acc ->
      Map.put(acc, variant_key(prefix, w), variant_url(url, w))
    end)
  end
  ...
  (`%{poster: variants(...)}`).
  """
  @spec variants(String.t() | nil, [width()]) :: map()
  ...
  defp variant_key(prefix, w), do: :"#{prefix}_w#{w}"
