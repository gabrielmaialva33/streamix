defmodule Streamix.Billing.Customers do
  ...
  def get_billing_customer(%{id: user_id}, provider) when is_integer(user_id) do
  ...
  def upsert_billing_customer!(%{id: user_id}, provider, external_id, metadata \\ %{})
      when is_integer(user_id) and is_binary(external_id) and external_id != "" do
    attrs = %{
      user_id: user_id,
    ...
    case Repo.get_by(BillingCustomer, user_id: user_id, provider: attrs.provider) do
      nil ->
        %BillingCustomer{}
